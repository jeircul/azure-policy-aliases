...
- Validate the config against the Renovate schema before returning it; fix any errors first.
- When defining regex/custom managers, rely on template fields (for example `datasourceTemplate`, `versioningTemplate`, `depNameTemplate`) rather than unsupported keys.
